                ...
                <div class="mt-2 p-4 w-100 bg-light shadow-lg rounded">
                    ...
                    <ul class="list-group bg-light">
                        ...
                        <li class="list-group-item">
                            <p class="text-dark fw-bold"><%=c.getUserbdByUserbdId().getUsername()%>
                            </p>
                            <p class="text-dark lead"><%=c.getText()%>
                            </p>
                        </li>
                        <%
                            }
                    ...
                    <form action="artworks-comment" method="POST" class="mt-lg-5 col-sm-8 justify-content-center text-center">
                        <input type="text" name="id"
                               value="<%= a.getId() %>"
                               hidden>
                        <input type="text" name="user-id"
                               value="<%= Autentication.isLogged(request) ? Autentication.getLoggedUser(request).getId() : null %>"
                               hidden>
                        <input type="text" class="form-control" name="comment"
                               placeholder="Express your ideas! <%= Autentication.isLogged(request) ? "(Comment as " + Autentication.getLoggedUser(request).getUsername() + ")" : "(Comment anonymously)" %>">
                        <input type="submit" class="btn btn-primary text-light fw-bold mt-4 w-25" value="Send">
                    </form>
                </div>
            </div>
        </div>
